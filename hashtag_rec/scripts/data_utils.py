...
def split_data_indices(data_size, data_types, data_probs, random_seed=0, fp_to_write=None):
...
def load_image_paths(file_path=os.path.join(DATA_DIR, IMG_PATHS_FILE_NAME)):
...
def load_image_tags(file_path=os.path.join(DATA_DIR, GT_TAGS_FILE_NAME)):
